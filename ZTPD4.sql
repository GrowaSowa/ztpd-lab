-- Oracle Spatial: SDO_GEOMETRY (07.11)
-- 1.
-- A.
CREATE TABLE figury (
	id NUMBER(1),
	ksztalt MDSYS.SDO_GEOMETRY
);


-- B.
INSERT INTO figury VALUES (1,
	MDSYS.SDO_GEOMETRY(2003, NULL, NULL,
		MDSYS.SDO_ELEM_INFO_ARRAY(1, 1003, 4),
		MDSYS.SDO_ORDINATE_ARRAY(5,7, 3,5, 5,3)
	)
);

INSERT INTO figury VALUES (2,
	MDSYS.SDO_GEOMETRY(2003, NULL, NULL,
		MDSYS.SDO_ELEM_INFO_ARRAY(1, 1003, 3),
		MDSYS.SDO_ORDINATE_ARRAY(1,1, 5,5)
	)
);

INSERT INTO figury VALUES (3,
	MDSYS.SDO_GEOMETRY(2002, NULL, NULL,
		MDSYS.SDO_ELEM_INFO_ARRAY(1,4,2, 1,2,1, 5,2,2),
		MDSYS.SDO_ORDINATE_ARRAY(3,2, 6,2, 7,3, 8,2, 7,1)
	)
);


-- C.
INSERT INTO figury VALUES (4,
	MDSYS.SDO_GEOMETRY(2003, NULL, NULL,
		MDSYS.SDO_ELEM_INFO_ARRAY(1, 1003, 4),
		MDSYS.SDO_ORDINATE_ARRAY(5,7, 6,7, 7,7)
	)
);


-- D.
SELECT id, SDO_GEOM.VALIDATE_GEOMETRY_WITH_CONTEXT(ksztalt, 0.0001) AS val FROM figury;


-- E.
DELETE FROM figury WHERE SDO_GEOM.VALIDATE_GEOMETRY_WITH_CONTEXT(ksztalt, 0.0001) NOT LIKE 'TRUE';


-- F.
COMMIT;